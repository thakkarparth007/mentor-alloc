...
// mail-template
var tmpl = Handlebars.compile( fs.readFileSync('mail_temp_cross.hbs').toString() );
...
function generate_mail(data) {
	return {
		...
		subject: "Mentees allotted so far.",
		html: tmpl(data)
	};
}

var data = {};
var o = "CHEM,CIVIL,CSE,ECE,EEE,ICE,MECH,MME,PROD".split(",");
var mail_count = 0;
var mail_queue = [];
for(var i in o) {
	var dept = o[i];
	data[dept] = JSON.parse(fs.readFileSync("./output-cross/" + dept + "-allotted.js"));
	//
	// DON'T SEND TO ARCHI!!!!
	//
...
/*
			...
			*/
			...
			//fs.writeFileSync('./email-cross/' + person.RollNum + '.html', generate_mail(person).html);
		}
	}
