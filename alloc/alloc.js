...
Alloc.categorize = function(list) {
	/* 
...
Alloc.allot = function(mentorsList, menteesList) {
	...
	var categorizedMentees = this.categorize(menteesList);

	var allotment = { };
	var unallocated = { };

	for(var dept in categorizedMentees) {
		if(!categorizedMentors[dept])
			return this.error("Error! Department Missing", dept, categorizedMentors, categorizedMentees);

		allotment[dept] = this.handleDepartment(categorizedMentors[dept], categorizedMentees[dept]);
		unallocated[dept] = {};

		["Male","Female"].forEach(function(gender) {
			unallocated[dept][gender] = [];
			for(var i = 0; i < categorizedMentees[dept][gender].length; i++) {
				if(categorizedMentees[dept][gender][i].Mentor === null) {
					unallocated[dept][gender].push(categorizedMentees[dept][gender][i]);
				}
			}
		});

		fs.writeFileSync('./output/' + dept.toUpperCase() + "-allotted.js", JSON.stringify(allotment[dept]));
		fs.writeFileSync('./output/' + dept.toUpperCase() + "-unallocated.js", JSON.stringify(unallocated[dept]));
	}
